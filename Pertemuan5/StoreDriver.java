...
public class StoreDriver {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        // Deklarasi variabel untuk total gaji di setiap divisi
        double totalManagerSalary = 0;
        double totalSalesAssociateSalary = 0;
        double totalShiftManagerSalary = 0;
        ...
        List<ShiftManager> shiftManagerList = new ArrayList<ShiftManager>();
        ...
        while (sc.hasNext()) {
            ...
            } else if (employeeType.equals("Shift")) {
                double numberOfShifts = sc.nextDouble();
                ShiftManager shiftManager = new ShiftManager(numberOfHoursWorked, hourlyRate, storeDetails, basePay, empName, numberOfShifts);
        ...
        // Hitung total gaji di setiap divisi dengan menggunakan method divisionSalary
        for (Manager manager : managerList) {
            totalManagerSalary += manager.divisionSalary();
             totalManagerSalary += manager.calculateBonusPay();
        }

        for (SalesAssociate salesAssociate : SalesAssociateList) {
            totalSalesAssociateSalary += salesAssociate.divisionSalary();
        }

        for (ShiftManager shiftManager : shiftManagerList) {
            totalShiftManagerSalary += shiftManager.divisionSalary();
        }
        }
        ...
        pwd.println("Total salary for managers: $" + totalManagerSalary);
        pwd.println("******************************************************");
        ...
        System.out.println("Total salary for sales associates: $" + totalSalesAssociateSalary);
        System.out.println("******************************************************");
        pwd.println("******************************************************");
        pwd.println("Number of employees working as SALES ASSOCIATES are: " + SalesAssociateList.size());
        pwd.println("Total salary for sales associates: $" + totalSalesAssociateSalary);
        pwd.println("******************************************************");
        ...
        for (SalesAssociate salesAssociate : SalesAssociateList) {
            ...
            System.out.println("Gross Payment: $" + salesAssociate.calculatePay());
             System.out.println("Bonus Pay: $" + salesAssociate.calculateBonusPay());
            pwd.println("Bonus Pay: $" + salesAssociate.calculateBonusPay());
        ...
        System.out.println("******************************************************");
        System.out.println("Number of employees working as SHIFT MANAGERS are: " + shiftManagerList.size());
        System.out.println("Total salary for shift managers: $" + totalShiftManagerSalary);
        System.out.println("******************************************************");
        pwd.println("******************************************************");
        pwd.println("Number of employees working as SHIFT MANAGERS are: " + shiftManagerList.size());
        pwd.println("Total salary for shift managers: $" + totalShiftManagerSalary);
        pwd.println("******************************************************");

        for (ShiftManager shiftManager : shiftManagerList) {
            shiftManagerCount++;
            System.out.println(shiftManagerCount + ". Shift Manager Details:");
            System.out.println(shiftManager);
            pwd.println(shiftManagerCount + ". Shift Manager Details:");
            pwd.println(shiftManager);
            System.out.println("Gross Payment: $" + String.format("%.1f", shiftManager.calculatePay()));
            pwd.println("Gross Payment: $" + String.format("%.1f", shiftManager.calculatePay()));
            System.out.print("Is " + shiftManager.getEmployeeName() + " eligible for promotion? ");
            System.out.println("Bonus Pay: $" + String.format("%.1f", shiftManager.calculateBonusPay()));
            pwd.println("Bonus Pay: $" + String.format("%.1f", shiftManager.calculateBonusPay()));
            if (shiftManager.checkPromotionEligibility()) {
                System.out.println("Yes, he/she is eligible\n");
            } else {
                System.out.println("No, he/she needs to work harder\n");
            }
